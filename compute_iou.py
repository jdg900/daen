...
def compute_mIoU(gt_dir, pred_dir, devkit_dir, dataset):
    ...
    if 'FZ' in dataset:
        ...
        image_path_list = join(devkit_dir, 'rgb_fog_val_filenames.txt')
        label_path_list = join(devkit_dir, 'gt_labelIds_fog_val_filenames.txt')
    ...
    if 'FZ' in dataset:
        ...
        mIoUs = np.delete(mIoUs, 16) # remove the class train beacuse it is not included in Foggy Zurich test set
