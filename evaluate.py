...
from dataset.ACDC_test import ACDCDataset
...
def eval():
    ...
    save_dir_acdc = osp.join(f'./result_ACDC', args.file_name)

    if not os.path.exists(save_dir_fz):
        os.makedirs(save_dir_fz)
    if not os.path.exists(save_dir_fd):
        os.makedirs(save_dir_fd)
    if not os.path.exists(save_dir_acdc):
        os.makedirs(save_dir_acdc)
    
    model.eval()
    device = torch.device('cuda:0')
    model.to(device)

    testloader1 = data.DataLoader(foggyzurichDataSet(args.data_dir_eval, args.data_list_eval, crop_size=(1920, 1080), mean=IMG_MEAN),
    ...
    testloader1 = data.DataLoader(ACDCDataset(args.data_dir_eval, args.data_list_eval_acdc, crop_size=(1920, 1080), mean=IMG_MEAN),
                                    batch_size=1, shuffle=False, pin_memory=True)
    if version.parse(torch.__version__) >= version.parse('0.4.0'):
        interp_eval = nn.Upsample(size=(1080,1920), mode='bilinear', align_corners=True)
    else:
    ...
    for index, batch in enumerate(testloader1):
        ...
        name = name[0].split('/')[-1]
        output.save('%s/%s' % (save_dir_acdc, name))
        output_col.save('%s/%s_color.png' % (save_dir_acdc, name[:-4]))
    miou_acdc = compute_mIoU(args.gt_dir_acdc, save_dir_acdc, args.devkit_dir_acdc, 'ACDC')
